    ...
    <div class="content">
        ...
        <div class="editPage">
            ...
            <div class="tab-ct-wrap">
                <form action="../update" method="post" id="cat-form">
                    <table class="table-module01" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
                            <th width="130" align="right">上级分类：</th>
                            <td valign="top">
                                <input type="text" name="parent.name" value="${model.parent.name}" class="parentName" readonly>
                                <input type="hidden" name="parent.id" class="parentId" value="${model.parent.id}">
                                <input type="hidden" name="path" class="path" value="${model.parent.path}">
                                <input type="hidden" name="pathNames" class="pathNames" value="${model.parent.pathNames}">
                                        ...
                                        <ul class="upload-queue" id="upload-queue">
                                            <li>
                                                <c:if test="${not empty model.picture}">
                                                    <input type="hidden" name="logo" value="${model.picture}" data-saved-url="${ctx}/${model.picture}"/>
                                                </c:if>
                        ...
                        <tr>
                            <th width="130" align="right">模型：<b class="color-red">*</b></th>
                            <td>
                                <select id="model" name="model.id" data-placeholder="请选择" class="select chosen-select ni">
                                    ...
                                    <c:forEach var="m" items="${models}">
                                        <option value="${m.id}"
                                                <c:choose>
                                                <c:when test="${m.id==model.model.id}">
                                                    selected
                                                        </c:when>
                                                    <c:otherwise>
                                                        <c:if test="${flag}">
                                ...
                                <c:if test="${flag}">
                                    <label style="color:red">分类模型已被商品引用</label>
                                </c:if>
                            ...
                            <td valign="top">
                                <select id="brands" name="brands" class="select chosen-select " data-placeholder="请选择关联品牌" multiple="multiple">
                                    <c:forEach var="brand" items="${brands}">
                                        ...
                                        ${fne:hasEqPropObj(model.brands, brand,"id")?"selected":""}
                                                    >${brand.name}</option>
                                    </c:forEach>
                                </select>
                        ...
                        <tr>
                            ...
                            <td valign="top">
                                ...
                                <input id="sort" name="sort" class="input" value="${model.sort}">
                        ...
                        <tr>
                            ...
                            <td>
                                <label><input name="visible" type="radio" value="1" ${model.visible?'checked':''}>可见</label>
                                <label><input name="visible" type="radio" value="0" ${model.visible?'':'checked'} >隐藏</label>
                            </td>
                        </tr>
