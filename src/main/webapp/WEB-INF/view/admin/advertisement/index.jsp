...
<div class="main">
    ...
    <div class="content">
        ...
        <div class="position">
            ...
            <a href="${ctx}/admin/advertisement/index"; title="广告管理">广告管理</a> &gt;
            列表
        </div>
        <!-- //End 当前位置 -->
        <div class="listPage">
...
<script type="text/javascript">
    ...
    var $g = $('#datagrid').jqGrid({
        url: 'list',
        datatype: 'json',
        colNames: ['操作', 'id','广告位名称','广告示意图','状态','平台','位置','添加时间'],
        colModel: [
            {
           ...
           // {name: 'statusType', index: 'statusType', template: 'text', sortable:true,search:true},
            {
                name: 'isShow',
                index: 'tu.status_type',
                template: 'text',
                width: 125,
                stype: "select",
                search: true,
	        ...
	        //{name: 'deviceType', index: 'deviceType', template: 'text', sortable:false,search:false},
	        {
                name: 'deviceType', index: 'tu.deviceType', template: 'select', stype: 'select',
                width: 150,
                sortable:false,
                searchoptions: {sopt: ["eq", "ne"], value: getSearchoptions(adTypes)},
                formatter: function (val) {
                    return formatByDictionary(val, adTypes);
                }
            },
            {name: 'posName', index: 'tu.posName', template: 'text', sortable:false,searchoptions: {sopt: ["cn"]}},
