...
$(".btn-cancel-wrap .btn-cancel").click(function () {
    window.location.href =contextPath+"/admin/discount/index";
});


var $g = $('#datagrid').jqGrid({
    url: contextPath+'/admin/activity/get-goodses',
    datatype: 'json',
    ...
    colModel: [
        ...
        { name: 'product.category', index: 'pCName', template: 'text', align: 'center',searchoptions: {sopt: ["cn"]},formatter:function(v){return v.name}},
    ...
    layer.open({
        ...
        yes: function (index) {
            ...
            $("#goods-list tr").each(function (i, el) {
                ...
                $(el).find('label').each(function(j, input) {
                    var $input = $(input);
                    $input.attr('for', $input.attr('for').replace(/(\[).*(?=])/, '$1' + i));
                });
            });
            layer.close(index);
        }
