var $g = $('#datagrid').jqGrid({
    url: 'list',
    datatype: 'json',
    colNames: ['操作', 'id','品牌名称','品牌别名','是否推荐','是否启用','品牌编号','LOGO'],
    colModel: [
        { template: 'actions2' },
        ...
        {
            name: 'recommend', index: 'recommend', formatter: function (va) {
            if (va) return "是"; else return "否";
        }, stype: 'select', searchoptions: {value: ':全部;0:否;1:是'}, align: 'left'
        },
        {
            name: 'enabled', index: 'enabled', formatter: function (va) {
            if (va) return "是"; else return "否";
        }, stype: 'select', searchoptions: {value: ':全部;0:否;1:是'}, align: 'left'
        },
        { name: 'brandCode', index: 'brandCode', template: 'text', align: 'left'},
        { name: 'logo', index: 'logo', template: 'img', align: 'center'}
    ],
    multiselect: true,
    autowidth: true,
    shrinkToFit: true,
