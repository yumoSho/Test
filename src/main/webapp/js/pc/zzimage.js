...
$(document).ready(function (){
    ...
    //    _index = $(".slider ul li").index($(this).parent());
    //    tag_pic();
    //    show_big_pic();
    //    //alert();
    //    return false;
    ...
    $slider.width($('.slider ul li').length * $slider_width);

    var slider_count = 0;

    //$('.btn-next').off("click").on("click",function() {
    //	if (_index >= $('.slider ul li').length - 1) {
    //		_index = 0;
    //		slider_count = 0;
    //	}
    //	else
    ...
    //		_index++;
    //		//索引超过4开始移动
    //		if(_index > 4 && _index > (slider_count +4))
    //		{
    //			slider_count++;
    //		}
    //	}
    ...
    //	if (_index <= 0) {
    //		_index = $('.slider ul li').length - 1;
    //		slider_count = $('.slider ul li').length - 5;
    //	}
    //	else
    //	{
    //		_index--;
    ...
    function tag_pic(){
        ...
        $(".slider ul li").eq(_index).addClass("current");
    }

    function show_big_pic(){
        $(".zoompic img").attr({ "src": $(".slider ul li").eq(_index).find(">img").attr("bigsrc"), "title": $(".slider ul li").eq(_index).find(">img").attr("title") });
...
(function($){
    $.fn.zzImage = function(opts){
        //.extend() 扩展jQuery类，添加zzImage方法，参数是对象类型{}
        opts = $.extend({}, $.fn.zzImage.opts, opts);
        this.each(function(){
            var boxwrap = $(this).find('.zz-box');//轮播元素父对象
            var thumbnail = slidewrap.find('li');//获取<li>对象集
